...
uint8 dataBuffer[BUFFER_SIZE]; // 接收数据缓冲区  
...
void LP_FLEXCOMM5_IRQHandler(void)
{
    ...
    while (kLPUART_RxDataRegFullFlag & LPUART_GetStatusFlags(USER_USART))  
    {  
        if (bufferIndex < BUFFER_SIZE)  // 确保缓冲区没有溢出  
        {  
            // 接受到的数据  
            dataBuffer[bufferIndex++] = LPUART_ReadByte(USER_USART);  
        }  
    }  
		extern void UART5_IRQHandler(uint8 *data);
		UART5_IRQHandler(dataBuffer);
		bufferIndex=0;
